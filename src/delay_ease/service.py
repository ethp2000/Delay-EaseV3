...
def display_status_message(ticket_data: dict):
    ...
    if status == "blocked_paper":
        ...
        if "learn_more" in ticket_data.get("next_action", ""):
            log.info(f"Learn more about {ticket_data.get('learn_more_topic', 'delay repay policies')}")
        ...
        if ticket_data.get("status") == "eligible":
            ...
            if is_type_a_toc(toc):
                ...
                try:
                    ...
                    log.info("You will receive a notification when compensation is ready for withdrawal")
